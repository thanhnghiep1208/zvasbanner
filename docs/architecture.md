...
- **Quản lý phiên**: `lib/clerk-sessions.ts` gọi Clerk Sessions API (`getSessionList`, `revokeSession`). User tự quản lý tại `/account/sessions` (`GET`/`DELETE` `/api/sessions`). Admin xem/thu hồi phiên user khác qua dashboard (`GET`/`DELETE` `/api/dashboard/users/sessions`, permission `block_user`, audit `session_revoke` trong `banner_events`).

...
- **Clerk `POST .../sign_ins` 409 (Conflict)** trong DevTools: thường do (1) đã có session Clerk còn sót cookie — thử đăng xuất, xóa cookie site + `*.clerk.accounts.dev`, reload; (2) bấm Sign up trong modal khi Clerk **tắt đăng ký** — app ẩn footer sign-in (`ClerkProvider` `appearance.elements.footer`); (3) tài khoản phải được tạo sẵn trong Clerk Dashboard (invite/admin), không tự đăng ký. Sau 409, refresh trang: đôi khi đã đăng nhập thành công một phầi.

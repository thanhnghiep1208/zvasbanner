...
11. **Xuất file** — export PNG/JPG qua Canvas API; có thể dùng `GET /api/proxy-image` nếu cần tải ảnh remote.
...
- Client timeout generate: **92s** (khớp server `maxDuration` 90s + buffer); không rút xuống 45s khi đã bật harmony pass.
...
4. Thử lỗi (timeout / quota) và xác nhận thông báo có mã lỗi ngắn.
